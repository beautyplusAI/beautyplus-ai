#!/usr/bin/env python3
"""
BeautyPlus skill CLI — single entry for agents:
preflight, install-deps, run-task, query-task, spawn-run-task,
last-task, history, resolve-input.
Run from anywhere: python3 /path/to/scripts/beautyplus_ai.py <command> ...
"""
...
STATE_DIR = Path.home() / ".openclaw" / "workspace" / "beautyplus-ai"
...
def _read_ak_sk_from_env_file() -> tuple[str, str]:
    ak = os.environ.get("BP_AK", "").strip()
    sk = os.environ.get("BP_SK", "").strip()
    ...
    try:
        with open(env_path, encoding="utf-8") as f:
            for line in f:
                ...
                if key == "BP_AK" and not ak:
                    ak = value
                elif key == "BP_SK" and not sk:
                    sk = value
    except OSError:
        pass
    return ak, sk
...
def _merge_pipeline_trace(
    ...
    *,
    ...
    """Attach upload/download/quota/submit steps for stdout JSON (stderr has [beautyplus-ai] lines)."""
...
def _save_submitted_checkpoint(task_name: str, input_src: str, task_id: str) -> None:
    ...
    rec = {
        ...
        "agent_instruction": (
            ...
            "`beautyplus_ai.py query-task --task-id` with the task_id above — do not "
...
def _run_task_command_argv(task: str, input_src: str, params_json: str) -> list[str]:
    argv = [
        "python3",
        str((SCRIPTS_DIR / "beautyplus_ai.py").resolve()),
        "run-task",
        "--task",
        task,
        "--input",
...
def build_spawn_run_task(
    ...
    """Build sessions_spawn payload for async BeautyPlus run-task (same as Medeo spawn-task pattern)."""
    script_path = str((SCRIPTS_DIR / "beautyplus_ai.py").resolve())
    ...
    if deliver_channel_str == "feishu":
        if is_video:
            delivery_instructions = (
                ...
                f"       --video /tmp/beautyplus_result.mp4 --to \"{deliver_to_str}\" \\\n"
        ...
        delivery_instructions = (
            ...
            "     ```\n"
            f"     curl -sL -o /tmp/beautyplus_result.{ext_hint} \"<result_url>\"\n"
            "     ```\n"
            f"     `message(action=\"send\", channel=\"discord\", target=\"{deliver_to_str}\", "
            f'filePath="/tmp/beautyplus_result.{ext_hint}")`\n'
    ...
    task_text = (
        "You are a BeautyPlus AI worker. Run the following command and complete delivery.\n\n"
        "BILLING (MANDATORY): This API consumes paid quota for the BP_AK tenant. Never tell the "
        ...
        "to the user; check BP_AK/BP_SK, network; timeouts: larger runTimeoutSeconds if needed.\n"
...
def cmd_preflight(_args: argparse.Namespace) -> int:
    ...
    if not ok:
        print(
            "BeautyPlus: obtain keys at https://beautyplus.com/developers "
            "and set BP_AK / BP_SK in scripts/.env (see .env.example).",
            file=sys.stderr,
        )
    return 0
...
def cmd_resolve_input(args: argparse.Namespace) -> int:
    ...
    dest = out_dir / f"beautyplus_in_{uuid.uuid4().hex[:10]}_{filename}"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="BeautyPlus skill CLI")
